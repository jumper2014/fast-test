<template>
  <div class="app-container" style="padding: 8px;">
    ...
    <el-row>
      ...
      <el-row :gutter="20">
        <el-col :span="4">
          <el-input v-model="remoteCopy.remoteDir" size="mini" placeholder="请指定远程目录" />
        </el-col>
      </el-row>
      <el-row>
        <el-col :span="24" :style="{ height: '20px' }" />
      ...
      <el-form ref="form" :model="form" size="small" label-width="80px">
        ...
        <el-form-item v-if="crud.status.add" label="上传">
          <el-upload
            ...
            :action="'/api/rfile' + '?name=' + form.name"
          >
            <div class="eladmin-upload"><i class="el-icon-upload" /> 添加文件</div>
            <div slot="tip" class="el-upload__tip">可上传*.*，且不超过100M</div>
          </el-upload>
        </el-form-item>
      </el-form>
...
import put from '@/api/fasttest/rfile'
...
export default {
  ...
  methods: {
    ...
    handleSuccess(response, file, fileList) {
      ...
      this.remoteCopy.fileName = response
      console.log(this.remoteCopy.fileName)
    ...
    handleCopy() {
      put(this.remoteCopy).then(response => {
        this.$message({
          message: '远程传输完成',
          type: 'success'
        })
